...
{
	[MAP_PIN_TYPE_ASSISTED_QUEST_CONDITION] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--9
	[MAP_PIN_TYPE_ASSISTED_QUEST_OPTIONAL_CONDITION] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--10
	[MAP_PIN_TYPE_ASSISTED_QUEST_ENDING] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--11
	[MAP_PIN_TYPE_ASSISTED_QUEST_REPEATABLE_CONDITION] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--12
	[MAP_PIN_TYPE_ASSISTED_QUEST_REPEATABLE_OPTIONAL_CONDITION] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--13
	[MAP_PIN_TYPE_ASSISTED_QUEST_REPEATABLE_ENDING] = "EsoUI/Art/Compass/quest_icon_assisted.dds",--14
	[MAP_PIN_TYPE_TRACKED_QUEST_CONDITION] = "EsoUI/Art/Compass/quest_icon.dds",--15
	[MAP_PIN_TYPE_TRACKED_QUEST_OPTIONAL_CONDITION] = "EsoUI/Art/Compass/quest_icon.dds",--16
	[MAP_PIN_TYPE_TRACKED_QUEST_ENDING] = "EsoUI/Art/Compass/quest_icon.dds",--17
	[MAP_PIN_TYPE_TRACKED_QUEST_REPEATABLE_CONDITION] = "EsoUI/Art/Compass/repeatableQuest_icon.dds",--18
	[MAP_PIN_TYPE_TRACKED_QUEST_REPEATABLE_OPTIONAL_CONDITION] = "EsoUI/Art/Compass/repeatableQuest_icon.dds",--19
	[MAP_PIN_TYPE_TRACKED_QUEST_REPEATABLE_ENDING] = "EsoUI/Art/Compass/repeatableQuest_icon.dds",--20
	[MAP_PIN_TYPE_QUEST_CONDITION] = "EsoUI/Art/Compass/quest_icon.dds", --21
	[MAP_PIN_TYPE_QUEST_OPTIONAL_CONDITION] = "EsoUI/Art/Compass/quest_icon.dds", --22
	[MAP_PIN_TYPE_QUEST_ENDING] = "EsoUI/Art/Compass/quest_icon.dds", --23
	[MAP_PIN_TYPE_QUEST_REPEATABLE_CONDITION] = "EsoUI/Art/Compass/repeatableQuest_icon.dds",--24
	[MAP_PIN_TYPE_QUEST_REPEATABLE_ENDING]= "EsoUI/Art/Compass/repeatableQuest_icon.dds",--25
    [MAP_PIN_TYPE_QUEST_REPEATABLE_OPTIONAL_CONDITION]= "EsoUI/Art/Compass/repeatableQuest_icon.dds",--26
    ...
    [MAP_PIN_TYPE_TRACKED_QUEST_ZONE_STORY_ENDING]= "EsoUI/Art/Compass/zoneStoryQuest_icon_assisted.dds",
    [MAP_PIN_TYPE_TRACKED_QUEST_ZONE_STORY_OPTIONAL_CONDITION]= "EsoUI/Art/Compass/zoneStoryQuest_icon_assisted.dds",
    [MAP_PIN_TYPE_QUEST_ZONE_STORY_CONDITION]= "EsoUI/Art/Compass/zoneStoryQuest_icon_assisted.dds",
    [MAP_PIN_TYPE_QUEST_ZONE_STORY_ENDING]= "EsoUI/Art/Compass/zoneStoryQuest_icon_assisted.dds",
    [MAP_PIN_TYPE_QUEST_ZONE_STORY_OPTIONAL_CONDITION]= "EsoUI/Art/Compass/zoneStoryQuest_icon_assisted.dds",
}
...
local breadcrumbQuestPinTextures =
{
	...
	[MAP_PIN_TYPE_QUEST_REPEATABLE_CONDITION] = "EsoUI/Art/Compass/quest_icon_door.dds",
	[MAP_PIN_TYPE_QUEST_REPEATABLE_ENDING]= "EsoUI/Art/Compass/quest_icon_door.dds",
    [MAP_PIN_TYPE_QUEST_REPEATABLE_OPTIONAL_CONDITION]= "EsoUI/Art/Compass/quest_icon_door.dds",
...
local ZoneExploration = ZO_MapPin.SUGGESTION_PIN_TYPES
...
function FyrMM.GetMapTextureFileInfo(mapId)
	local tileTexture = (GetMapTileTextureForMapId(mapId, 1)):lower()
	if tileTexture == nil or tileTexture == "" then
		return "tamriel_0", "tamriel_", "art/maps/tamriel/"
	end
	local pos = select(2,tileTexture:find("maps/([%w%-]+)/"))
...
local function SetDigSitePoints(digSite)
	...
	for i, point in ipairs(borderInformation.borderPoints) do
		if RotateMap then
			if CurrentMap.Heading ~= nil and CurrentMap.PlayerNX ~= nil then
				local ix = point.x -digSite.nX
				local iy = point.y -digSite.nY
				local rx = (math.cos(-CurrentMap.Heading) * ix) - (math.sin(-CurrentMap.Heading) * iy)
				local ry = (math.sin(-CurrentMap.Heading) * ix) + (math.cos(-CurrentMap.Heading) * iy)
				digSite:AddPoint(rx, ry)
			end
		else
			digSite:AddPoint(point.x, point.y)
		end
	end
end
...
local function ProcessQuestPinClick(pin)
	...
	local entries = {}
	local entry = ""
	Handler.Callback = function (questIndex) FOCUSED_QUEST_TRACKER:ForceAssist(questIndex) end
...
local function PinOnMouseEnter(pin)
	...
	if pin.m_PinType ~= nil then
		...
		if ZoneExploration[pin.m_PinType] then
...
function FyrMM.PlaceBorderPins()
	...
	if FyrMM.SV.BorderTreasures then
		if not table.empty(Treasures) then
		...
		if not table.empty(Digsites) then
...
local function CreateDigSiteBlob(DigsiteId)
	...
	if isShownInCurrentMap then
		...
		if digSite == nil then
			digSite = WINDOW_MANAGER:CreateControl("Fyr_MM_Scroll_Map_DigSite"..tostring(FyrMM.currentDigSiteCount), Fyr_MM_Scroll_Map_QuestPins, CT_POLYGON)
			digSite.nDistance = function(self) if self.nX == nil then return 1 end return math.sqrt((zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))+(zo_round(CurrentMap.PlayerNY*10000)-zo_round(self.nY*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nY*10000)))/10000 end
		...
		local centerColor = ZO_MAP_PIN_TRACKED_DIG_SITE_COLOR
		...
		FyrMM.SetPinAnchor(digSite, centerX, centerZ, Fyr_MM_Scroll_Map_QuestPins)
...
function FyrMM.HideCheck() -- fires every 100 ticks
	...
	local crownStoreActive = WINDOW_MANAGER:IsSecureRenderModeEnabled()  or not ZO_ProvisionerTopLevelNavigationContainerScroll:IsHidden() or not ZO_SharedRightPanelBackground:IsHidden()
	local frameRate = GetFramerate()
	if frameRate < frameRatePrevious and frameRatePrevious - frameRate > 24 and frameRate < 20 then
...
function FyrMM.SetMapToPlayerLocation(stealth)
	...
	if not ZO_WorldMap:IsHidden() or not ZO_QuestJournal:IsHidden() then return end
...
local function ZoneCheck()
	if ZO_WorldMap:IsHidden() then
		...
		if CurrentMap.filename ~= filename then
			...
			CancelUpdates()
			local ZoneId = 0
			if FyrMM.MapSizes[FyrMM.GetMapId()] then
...
function FyrMM.PinUpdate()
	...
	if FyrMM.CustomPinCount ~= AvailableCustomPins() or table.empty(FyrMM.CustomPinList) or detectedNewCustomPin then
		...
		if table.empty(FyrMM.CustomPinList) then
			FyrMM.LoadCustomPinList()
		end
...
local function SpeedMeasure()
	FyrMM.WaypointPins()
	if table.empty(PositionLog3D) then return end
	local t1 = PositionLog3D[1][5]
	local x13d = PositionLog3D[1][2]
	local y13d = PositionLog3D[1][3]
	local z13d = PositionLog3D[1][4]
	local x12d = PositionLog3D[1][6]
	local y12d = PositionLog3D[1][7]
	local d3d = 0
	local d2d = 0
	local v13d = 0
	local va3d = 0
	local i = 0
	local size = 1
	if FyrMM.MapSizes[GetCurrentMapId()] then
		size = FyrMM.MapSizes[GetCurrentMapId()]
	else
		size = FyrMM.currentMap.TrueMapSize
	end
	if IsPlayerMoving() then
		for i=2, PositionLogCounter do
			d3d = math.sqrt((PositionLog3D[i][2]-x13d)*(PositionLog3D[i][2]-x13d)+(PositionLog3D[i][3]-y13d)*(PositionLog3D[i][3]-y13d)+(PositionLog3D[i][4]-z13d)*(PositionLog3D[i][4]-z13d))
			v13d = 0.1487 * d3d / math.abs((PositionLog3D[i][5]-t1)/1000)
			x13d = PositionLog3D[i][2]
			y13d = PositionLog3D[i][3]
			z13d = PositionLog3D[i][4]
			x12d = PositionLog3D[i][6]
			y12d = PositionLog3D[i][7]
			t1 = PositionLog3D[i][5]
			va3d = va3d + v13d
		end
		i = PositionLogCounter
		if i == 0 then return end
		local nsize = 0.0143*math.sqrt((PositionLog3D[i][2]-PositionLog3D[1][2])*(PositionLog3D[i][2]-PositionLog3D[1][2])+(PositionLog3D[i][3]-PositionLog3D[1][3])*(PositionLog3D[i][3]-PositionLog3D[1][3]))/math.sqrt((PositionLog3D[i][6]-PositionLog3D[1][6])*(PositionLog3D[i][6]-PositionLog3D[1][6])+(PositionLog3D[i][7]-PositionLog3D[1][7])*(PositionLog3D[i][7]-PositionLog3D[1][7]))
		if nsize ~= 1e309 and size < nsize then
			FyrMM.currentMap.TrueMapSize = nsize
		end
		va3d = va3d / (PositionLogCounter - 1)
	end
	PositionLogCounter = 0
	if FyrMM.MovementSpeedPrevious ~= nil then
		FyrMM.MovementSpeed = (va3d + FyrMM.MovementSpeedPrevious) / 2
	else
	...
	if type(p) == "table" then
		...
		if enabled then
			...
			if type(p.m_PinTag) == "table" then
				if p.m_PinTag.IsAvailableQuest or p.m_PinTag.isZoneStory then
					p.m_PinTag.IsAvailableQuest = true
...
function FyrMM.WaypointPins()
	...
	if wpx ~=0 or wpy ~= 0 then
		...
		if waypointPin == nil then
			waypointPin = WINDOW_MANAGER:CreateControl("Fyr_MM_Scroll_Map_Pins_PlayerWaypoint", Fyr_MM_Scroll_Map_Pins, CT_TEXTURE)
			waypointPin.nDistance = function(self) if self.nX == nil then return 1 end return math.sqrt((zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))+(zo_round(CurrentMap.PlayerNY*10000)-zo_round(self.nY*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nY*10000)))/10000 end
		...
		waypointPin.MapId = FyrMM.GetMapId()
		waypointPin.nX = wpx
		waypointPin.nY = wpy
	...
	if wpx ~=0 or wpy ~= 0 then
		...
		if RallyPin == nil then
			RallyPin = WINDOW_MANAGER:CreateControl("Fyr_MM_Scroll_Map_Pins_RallyPoint", Fyr_MM_Scroll_Map_Pins, CT_TEXTURE)
			RallyPin.nDistance = function(self) if self.nX == nil then return 1 end return math.sqrt((zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))+(zo_round(CurrentMap.PlayerNY*10000)-zo_round(self.nY*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nY*10000)))/10000 end
	...
	if wpx ~=0 or wpy ~= 0 then
		...
		if PingPin == nil then
			...
			PingPin.nDistance = function(self) if self.nX == nil then return 1 end return math.sqrt((zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nX*10000))+(zo_round(CurrentMap.PlayerNY*10000)-zo_round(self.nY*10000))*(zo_round(CurrentMap.PlayerNX*10000)-zo_round(self.nY*10000)))/10000 end
	...
	if CurrentMap.filename ~= "giantsrun_base_0" then
		for nodeIndex = 1, GetNumFastTravelNodes() do
			...
			if known or FyrMM.SV.ShowUnexploredPins then
				if not known and poiType ~= POI_TYPE_HOUSE and poiType ~= POI_TYPE_GROUP_DUNGEON then icon = "/esoui/art/icons/poi/poi_wayshrine_incomplete.dds" glowIcon = "/esoui/art/icons/poi/poi_wayshrine_glow.dds" end
				if not known and poiType == POI_TYPE_HOUSE then icon = "/esoui/art/icons/poi/poi_group_house_unowned.dds" glowIcon = "/esoui/art/icons/poi/poi_group_house_unowned_glow.dds" end
				if not known and poiType == POI_TYPE_GROUP_DUNGEON then icon = "/esoui/art/icons/poi/poi_groupinstance_incomplete.dds" glowIcon = "/esoui/art/icons/poi/poi_groupinstance_incomplete_glow.dds" end
            ...
            if pinType ~= MAP_PIN_TYPE_INVALID then
                ...
                if isInCurrentMap then
					...
					if GetControl("Fyr_MM_Scroll_Map_EventUnits_"..unitTag) == nil then
						pin = WINDOW_MANAGER:CreateControl("Fyr_MM_Scroll_Map_EventUnits_"..unitTag, Fyr_MM_Scroll_Map_EventUnits, CT_TEXTURE)
						pin:SetHandler("OnMouseEnter", PinOnMouseEnter)
						pin:SetHandler("OnMouseExit", PinOnMouseExit)
					end
                    FyrMM.SetEventPin(pinType, unitTag, tag)
                end
            end
        end
    end
...
function FyrMM.SetEventPin(pinType, tag, ntag)
	local pin, nX, nY
...
function FyrMM.RefreshGroup()
	if not FyrMM.GroupRefreshNeeded and not IsCompanionAround then return end
...
function FyrMM.UpdateKeepNetworkPositions()
	if not IsInAvAZone() or Fyr_MM:IsHidden() or not CurrentMap.ready then return end
	local  mWidth, mHeight = Fyr_MM_Scroll_Map:GetDimensions()
	local Count, l, startX, startY, endX, endY
	for i=1, 100 do
		l = GetControl("Fyr_MM_Scroll_Map_Links_Link"..tostring(i))
...
function FyrMM.UpdateKeepNetwork()
	if FyrMM.Halted then return end
	if not IsInAvAZone() or (not FyrMM.Visible or Fyr_MM:IsHidden()) or (not CurrentMap.ready and not CurrentMap.mapBuilt) then return end
	...
	if(IsLocalBattlegroundContext(obgContext)) then	
		if(ZO_WorldMap_IsObjectiveShown(okeepId, objectiveId, obgContext)) then 
				...
				if returnPinType ~= MAP_PIN_TYPE_INVALID then
					local objectiveReturnControl = GetControl("Fyr_MM_Scroll_Map_Objectives_ObjectiveReturn"..tostring(objectiveId))
					if objectiveReturnControl == nil then
						objectiveReturnControl = WINDOW_MANAGER:CreateControl("Fyr_MM_Scroll_Map_Objectives_ObjectiveReturn"..tostring(objectiveId), Fyr_MM_Scroll_Map_Objectives, CT_TEXTURE)
				...
				if(opinType ~= MAP_PIN_TYPE_INVALID) then
					...
					if objectiveControl then
						local auraPinType = GetObjectiveAuraPinInfo(okeepId, objectiveId, bgContext)
...
function FyrMM.InitialPreload()
	...
	EVENT_MANAGER:RegisterForUpdate("OnFyrMiniMapInitialPreload", 30, function()
		...
		if task == 10 and IsInAvAZone() then FyrMM.RequestKeepRefresh() end
		if task == 11 then FyrMM.PinUpdate() FyrMM.UpdateAntiquityDigSites() end
...
local function CreatePin(obj, pinType, pinTag, x, y, radius, borderInformation)
	...
	local key = ""
	local r = {}
	if PRCustomPins[pinType] or ZoneExploration[pinType] then
...
local function OnInit() -- Initialize Map and Update events after add-on load
	...
	Fyr_MM_Time:SetHandler("OnMouseUp", function() if FyrMM.TimeFormat == 1 then FyrMM.TimeFormat = 2 elseif FyrMM.TimeFormat == 2 then FyrMM.TimeFormat = 0 else FyrMM.TimeFormat = 1 end FyrMM.SV.TimeFormat = FyrMM.TimeFormat return end)
	...
	FOCUSED_QUEST_TRACKER:RegisterCallback("QuestTrackerAssistStateChanged", function() FyrMM.UpdateQuestPinPositions() end)
	FOCUSED_QUEST_TRACKER:RegisterCallback("QuestTrackerRefreshedMapPins", function() FyrMM.RequestQuestPinUpdate() end)
	...
	ZO_PreHook(ZO_MapPin, "SetLocation", function(ref, xLoc, yLoc, Radius) SetPinLocation(ref, xLoc, yLoc, Radius) end)
...
EVENT_MANAGER:RegisterForEvent( "MiniMap", EVENT_ANTIQUITY_DIG_SITES_UPDATED, function(eventCode, antiquityId) FyrMM.UpdateAntiquityDigSites() end)