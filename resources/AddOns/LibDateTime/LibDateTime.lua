...
function lib:GetTraderWeek(weekOffset) -- TODO update usecases
    ...
    if(endTime == 0) then return end -- this happens when the guild system is down 
...
function lib:CalculateDayOffset(timestamp)
    return mfloor((ostime() - timestamp) / SECONDS_PER_DAY)
end

function lib:GetTraderDay(timeStamp)
    local _, cycleTime = GetGuildKioskCycleTimes()
    if(cycleTime == 0) then return end -- this happens when the guild system is down 
    timeStamp = timeStamp or GetTimeStamp()

    -- remove the current time of day and add the seconds of the cycle time instead
    local startTime = timeStamp - (timeStamp % SECONDS_PER_DAY) + (cycleTime % SECONDS_PER_DAY)
    local endTime = startTime + SECONDS_PER_DAY
